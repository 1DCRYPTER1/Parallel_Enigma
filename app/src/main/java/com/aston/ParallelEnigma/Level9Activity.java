...
public class Level9Activity extends Activity {
	...
	private void initialize() {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (edittext1.getText().toString().toLowerCase().equals("saturday")) {
					dialog.create().show();
				}
			}
	...
	private void initializeLogic() {
		setTitle("Level9:The Day Of The Dead");
		next.setClass(getApplicationContext(), Level10Activity.class);
		...
		dialog.setPositiveButton("Yes✔️", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface _dialog, int _which) {
				savegame.edit().putString("level", "10").commit();
				startActivity(next);
				finish();
			}
		});
