...
public class Level3Activity extends Activity {
	...
	private void initialize() {
		...
		edittext1.addTextChangedListener(new TextWatcher() {
			...
			public void onTextChanged(CharSequence _param1, int _param2, int _param3, int _param4) {
				...
				if (edittext1.getText().toString().toLowerCase().equals("enigma")) {
	...
	private void initializeLogic() {
		button1.setVisibility(View.INVISIBLE);
		setTitle("Level3:Code Paste");
		next.setClass(getApplicationContext(), Level4Activity.class);
		dialog.setTitle("Success");
		dialog.setMessage("Hence forward keep an eye on your clipboard (Paste).\nAre you ready for next level?");
		dialog.setPositiveButton("Yes✔️", new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface _dialog, int _which) {
				savegame.edit().putString("level", "4").commit();
				startActivity(next);
				finish();
			}
		});
		...
		((ClipboardManager) getSystemService(getApplicationContext().CLIPBOARD_SERVICE)).setPrimaryClip(ClipData.newPlainText("clipboard", "ENIGMA"));
