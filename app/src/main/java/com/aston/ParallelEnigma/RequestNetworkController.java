...
public class RequestNetworkController {
	...
	private OkHttpClient getClient() {
		if (client == null) {
			...
			try {
				...
				builder.hostnameVerifier(new HostnameVerifier() {@Override
	...
	public void execute(final RequestNetwork requestNetwork, String method, String url, final String tag, final RequestNetwork.RequestListener requestListener) {
		...
		if (requestNetwork.getHeaders().size() > 0) {
			HashMap < String,
			Object > headers = requestNetwork.getHeaders();
		...
		try {
			if (requestNetwork.getRequestType() == REQUEST_PARAM) {
				if (method.equals(GET)) {
					...
					if (requestNetwork.getParams().size() > 0) {
						HashMap < String,
						Object > params = requestNetwork.getParams();
			...
			getClient().newCall(req).enqueue(new Callback() {@Override
				public void onFailure(Call call, final IOException e) {
					requestNetwork.getActivity().runOnUiThread(new Runnable() {@Override
						public void run() {
